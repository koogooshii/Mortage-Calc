...
export class MortgageService {
  ...
  public generateScheduleAndSummary(params: MortgageParams): {
    ...
    const finalSummary: MortgageSummary = {
      ...
      balanceAtEndOfTerm,
      interestSavedLifetime: fullSummary.interestSavedLifetime,
      interestSavedOverTerm: interestSavedOverTerm > 0 ? interestSavedOverTerm : 0,
      timeSaved: timeSaved,
      extraPaymentsByYear: fullSummary.extraPaymentsByYear,
      initialPeriodicPayment: fullSummary.initialPeriodicPayment,
      recurringExtraPayment: 0, // Will be calculated below
      initialTotalPeriodicPITI: 0, // Will be calculated below
      periodicTaxesAndInsurance: periodicTax + periodicInsurance + periodicPMI,
    ...
    // Calculate recurring extra payments that match the loan's frequency
    const recurringExtraPayment = params.recurringPayments
      .filter(p => p.frequency === params.paymentFrequency)
      .reduce((sum, p) => sum + p.amount, 0);

    finalSummary.recurringExtraPayment = recurringExtraPayment;
    finalSummary.totalPeriodicPITI += recurringExtraPayment;
    finalSummary.initialTotalPeriodicPITI = finalSummary.initialPeriodicPayment + finalSummary.periodicTaxesAndInsurance + recurringExtraPayment;

    // Adjust monthly equivalent
    switch (params.paymentFrequency) {
      case 'weekly':
      case 'accelerated-weekly':
        finalSummary.totalMonthlyPITIEquivalent += recurringExtraPayment * 52 / 12;
        break;
      case 'bi-weekly':
      case 'accelerated-bi-weekly':
        finalSummary.totalMonthlyPITIEquivalent += recurringExtraPayment * 26 / 12;
        break;
      case 'monthly':
      default:
        finalSummary.totalMonthlyPITIEquivalent += recurringExtraPayment;
        break;
    }
  ...
  private _performCalculation(params: MortgageParams, limitToTerm: boolean): {
    ...
    const summary: MortgageSummary = {
      ...
      initialTotalPeriodicPITI: 0,
      periodicTaxesAndInsurance: 0,
